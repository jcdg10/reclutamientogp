        ...
        <table>
            ...
            <tbody>
                ...
                <tr class="borde">
                    ...
                    <td><b>$ {{ number_format($montoCredito,2) }}</b></td>
            ...
            <tbody style="font-size: 12px;">
                ...
                <?php 
                    ...
                    foreach ($simulations as $s) {
                        ...
                        $apertura = $montoCredito * ($s->comision_apertura / 100);



                        if($s->avaluo_banco == '' || $s->avaluo_banco == null){

                            $avaluo = $garantia * $s->costo_avaluo;

                        }

                        else{

                            $avaluo = $s->avaluo_banco;

                        }

                        

                        $intereses = ((($s->tasa_interes/100) / 365) * 31) * $montoCredito;

                        $amortizacion = (($montoCredito * $s->pago_x_mil) / 1000) - $intereses;

                        $seguroVida = $montoCredito * $s->seguro_vida;

                        $seguroDano = $garantia * $s->seguro_danos;

                        $seguroDesempleo = $montoCredito * $s->seguro_desempleo;

                        $comisionAdministracion = $s->comision_admon;

                        $iva = 0.00;

                        if($s->destination_id == 5){

                            $iva = $intereses * ($s->iva / 100);

                        }



                        $mensualidad = $intereses + $amortizacion + $seguroVida + $seguroDano + $seguroDesempleo + $comisionAdministracion + $iva;
                    ...
                    echo '
                        ...
                        <tr class="borde">
                            ...
                            <td class="alignright">$ '.number_format($apertura,2).'</td>

                            <td class="alignright">$ '.number_format($avaluo,2).'</td>

                            <td class="lastrow alignright">$ '.number_format($mensualidad,2).'</td>
